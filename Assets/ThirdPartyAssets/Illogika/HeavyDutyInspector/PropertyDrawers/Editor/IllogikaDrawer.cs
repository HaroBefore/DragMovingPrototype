...
namespace HeavyDutyInspector
{
	...
	public class IllogikaDrawer : PropertyDrawer {
        ...
        public static string FormatLikeInspector(string label)
        {
            int firstUnderscore = label.IndexOf('_');
            if (firstUnderscore > -1 && firstUnderscore < 2)
                label = label.Substring(firstUnderscore + 1);

            for(int i = label.Length - 1; i >= 0; --i)
            {
                if (char.IsUpper(label[i]) && i > 0 && label[i - 1] != '_')
                ...
                if (i == 0)
                {
                    ...
                    label = label.Insert(0, c.ToString());
                }
            }
